function [] = gcIm2Nrrd(datadir,rot)
% Saves averaged stacks for the imaged planes onto nrrd 16bit format which
...
disp(['This is gcIm2nrrd, processing ' datadir '...'])
...
waitbar(0.5,h,'Saving nrrd...');
if ~exist(fullfile(datadir,'registration'),'dir')
    mkdir(fullfile(datadir,'registration'));
end

% Save
nrrdWriter(fullfile(datadir,'registration',[gm.name '_01.nrrd']), ...
    stk, pxdim, [0 0 0], 'raw');
